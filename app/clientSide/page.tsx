                    ...
                    <div className="grid gap-[30px] sm:grid-cols-2 lg:grid-cols-3">
                        {data.map((products, index) => (
                            <div key={index} className="w-full bg-[white] shadow-lg p-[20px] rounded-[8px] hover:shadow-xl hover:scale-[1.05] transition-all">
                                ...
                                <p className="mt-[50px] mb-[8px] text-[green] capitalize">{products.category}</p>
                                <p className="font-bold text-[orange] [10px] mb-[8px]">{products.title}</p>
                                <p className="line-clamp-2 text-[#0b2450] mb-[8px]">{products.description}</p>
                                <p className="text-[20px] text-[black] font-semibold mb-[8px]">${products.price}</p>
                                <p className="text-[#a88441] text-[18px] flex items-center mb-[10px]"><IoIosStar className="text-[23px] mr-[5px]" /><span className="mr-[5px]">{products.rating.rate}</span>({products.rating.count} reviews)</p>
